...
const cryptoCoins = ['Bitcoin', 'Ethereum', 'Cardano', 'Polkadot', 'Solana'];
const durationOptions = ["every day", "every week"];

export default function OrderForm({grantPermissions}: {grantPermissions: (allowance: bigint, period: number) => void}) {
  const [selectedCoin, setSelectedCoin] = useState('');
  const [order, setOrder] = useState('Buy me something nice');
  ...
  return (
    <div className="max-w-md mx-auto mt-96 p-6 bg-green-200 text-black rounded-lg shadow-lg" style={{ backgroundColor: 'rgba(235, 241, 63, 0.87)' }}   >
      ...
      <div className="flex space-x-4 mb-6">
        <div className="flex-1 relative">
          ...
          />
          <span className="absolute right-8 top-2 text-gray-500 font-bold">ETH</span>
        </div>
      ...
      <button
        ...
        onClick={() => grantPermissions(parseEther(budget), optionToPeriod(duration))}
